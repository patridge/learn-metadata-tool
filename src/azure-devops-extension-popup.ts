...
let uidSpan = document.getElementById("uid") as HTMLSpanElement;
let contentUrl = document.getElementById("contentUrl") as HTMLAnchorElement;
let relatedFeedbackWorkItemsQueryUrl = document.getElementById("relatedWorkItemsQueryUrl") as HTMLAnchorElement;
let relatedVerbatimsWorkItemsQueryUrl = document.getElementById("relatedVerbatimsQueryUrl") as HTMLAnchorElement;
let msAuthorSpan = document.getElementById("msAuthor") as HTMLSpanElement;
let msDateSpan = document.getElementById("msDate") as HTMLSpanElement;
let contentYamlSourceSpan = document.getElementById("yamlSourceSpan") as HTMLSpanElement;
let contentYamlGitUrlAnchor = document.getElementById("repoUrlYaml") as HTMLAnchorElement;
let contentMarkdownSourceSpan = document.getElementById("markdownSourceSpan") as HTMLSpanElement;
let contentMarkdownGitUrlAnchor = document.getElementById("repoUrlMarkdown") as HTMLAnchorElement;
let contentNotebookSourceSpan = document.getElementById("notebookSourceSpan") as HTMLSpanElement;
let contentNotebookGitUrlAnchor = document.getElementById("repoUrlNotebook") as HTMLAnchorElement;
let customLink = document.getElementById("customLink") as HTMLAnchorElement;
...
let copyButtons = Array.from(document.getElementsByClassName("copy-field-btn")) as HTMLElement[];
copyButtons.forEach(function (btn) {
    btn.addEventListener("click", async function(_element) {
        ...
        let siblingCopyTargets = Array.from(btn.parentElement!.parentElement!.getElementsByClassName("copy-field-target"));
        let copyTarget = siblingCopyTargets && siblingCopyTargets[0] as HTMLElement | undefined;
...
let displayWorkItemData = async function (workItemData: WorkItemData): Promise<void> {
     ...
     * @type {string}
     */
    let uid: string = workItemData.UID || workItemData["Module UID"] || workItemData["Learning Path UID"];
    if (uid) {
    ...
    let learnUrl: string = workItemData.URL || workItemData["Published URL"];
    if (learnUrl) {
...
let displayContentPageMetadata = function (metadata: PageMetadata) {
...
let getCurrentPageMetadata = function (rootElement: Document): PageMetadata {
            ...
            if (notebookPublicUrl.startsWith("https://raw.githubusercontent.com/")) {
                ...
                if (notebookPublicUrl.startsWith("https://learn.microsoft.com/training/")) {
                    ...
                    if (gitYamlEditUrl) {
                        ...
                        // e.g., "https://learn.microsoft.com/en-us/training/modules/intro-computer-vision-tensorflow/3-train-dense-neural-networks" => "intro-computer-vision-tensorflow/3-train-dense-neural-networks"
                        ...
                        // e.g., "https://learn.microsoft.com/training/modules/intro-computer-vision-tensorflow/notebooks/3-train-dense-neural-networks.ipynb" => "intro-computer-vision-tensorflow/notebooks/6-transfer-learning.ipynb"
                        ...
                        // e.g., "https://github.com/MicrosoftDocs/learn-pr/blob/main/learn-pr/tensorflow/intro-computer-vision-tensorflow/6-transfer-learning.yml#L16" => "https://github.com/MicrosoftDocs/learn-pr/blob/main/learn-pr/tensorflow/intro-computer-vision-tensorflow/notebooks/6-transfer-learning.ipynb"
                    }
                }
            }
        }
...
chrome.runtime.onMessage.addListener(function (request: any, sender: chrome.runtime.MessageSender, sendResponse: (response: any) => void) {
...
let getCustomLink = async function (): Promise<any> {
...
let displayCustomLink = async (): Promise<void> => {
